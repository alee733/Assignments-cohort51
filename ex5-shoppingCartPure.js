...
function addToShoppingCart(cart, item) {
  ...
  // Ensure the array contains at most three items
  return newCart.length > 3 ? newCart.slice(newCart.length - 3) : newCart;
}

// ! Test functions (plain vanilla JavaScript)
function test1() {
  console.log('Test 1: addToShoppingCart should take two parameters');
  console.assert(addToShoppingCart.length === 2, 'Test 1 Failed');
}
...
function test2() {
  ...
  console.assert(
    ...
    'Test 2 Failed: Results are not identical for the same input'
  );
  console.assert(
    JSON.stringify(initialCart) === JSON.stringify(['bananas', 'milk']),
    'Test 2 Failed: Input array was mutated'
  );
}
...
function test3() {
  ...
  console.assert(result.length === 3, 'Test 3 Failed: Cart length is incorrect');
  console.assert(
    result.includes('chocolate'),
    'Test 3 Failed: Chocolate was not added'
  );
}
...
function test4() {
  ...
  console.assert(result.length === 3, 'Test 4 Failed: Cart length is incorrect');
  console.assert(
    result.includes('waffles'),
    'Test 4 Failed: Waffles were not added'
  );
}
